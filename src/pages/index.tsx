...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // 6. Fetch clinical records to find "patients to observe"
      ...
      if (clinicalRecords) {
        const toObserve = clinicalRecords
          .filter(r => r.observaciones_generales && r.observaciones_generales.trim().length > 3)
          .map(r => {
            const pat: any = Array.isArray(r.patient) ? r.patient[0] : r.patient;
            return {
              id: pat?.id,
              name: pat?.full_name || 'Paciente',
              ...
              {loading ? (
                ...
                <div className="space-y-3">
                  {patientsToObserve.map((obs) => (
                    <div key={obs.id} className="bg-surface-container-low p-3 rounded-lg border border-outline-variant/20 space-y-1">
