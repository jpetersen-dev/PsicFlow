...
export default function Calendario() {
  ...
  const handleCreatePersonalEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error al crear evento personal: ' + err.message);
    }
  };
                        ...
                        >
                          ...
                          <div className="space-y-1 flex-1 overflow-y-auto max-h-24 hide-scrollbar">
                            ...
                            {daySessions.map((sess) => {
                              ...
                              return (
                                <Link 
                                  ...
                                  className={`appointment-card block p-1 rounded text-[9px] leading-tight font-medium border-l-4 truncate ${getSessionColorClass(sess.modality, sess.status_session)}`}
                            ...
                            {dayEvents.map((evt) => (
                              <div
                                key={evt.id}
                                className={`block p-1 rounded text-[9px] leading-tight font-medium border-l-4 truncate ${getEventColorClass(evt.event_type)}`}
              ...
              {calendarView === 'week' && (
                <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
                  {weekDays.map((day, idx) => {
                    ...
                    const daySessions = sessions.filter(s => s.date_session === dateStr);
                            ...
                            <>
                              {daySessions.map(sess => (
                                <Link 
                                  ...
                                  href={`/sesiones/${sess.id}`}
                                  onClick={(e) => e.stopPropagation()}
                                  className={`block p-2 rounded-lg text-[10px] leading-snug font-semibold border-l-4 shadow-sm hover:scale-[1.02] transition-all ${getSessionColorClass(sess.modality, sess.status_session)}`}
                                >
                              ...
                              {dayEvents.map(evt => (
                                <div 
                                  key={evt.id}
                                  className={`p-2 rounded-lg text-[10px] leading-snug font-semibold border-l-4 shadow-sm ${getEventColorClass(evt.event_type)}`}
                                >
                                  <div className="flex justify-between items-center mb-0.5">
              ...
              {calendarView === 'day' && (
                <div className="bg-surface-container-lowest rounded-2xl shadow-sm border border-outline-variant/20 p-6 space-y-6">
                  ...
                  {(() => {
                    ...
                    const daySessions = sessions.filter(s => s.date_session === selDateStr);
                          ...
                          if (item.type === 'session') {
                            ...
                            return (
                              <div key={`timeline-sess-${sess.id}-${idx}`} className="relative">
                                ...
                                <div className="bg-surface-container-low p-4 rounded-xl border border-outline-variant/15 shadow-sm hover:border-primary/30 transition-all flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                            ...
                            return (
                              <div key={`timeline-evt-${evt.id}-${idx}`} className="relative">
                                ...
                                <div className="bg-surface-container-low p-4 rounded-xl border border-outline-variant/15 shadow-sm flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                  ...
                                  <button
                                    onClick={() => handleDeletePersonalEvent(evt.id)}
                                    className="text-on-surface-variant hover:text-danger p-2 hover:bg-surface-container rounded-lg transition-colors cursor-pointer self-end sm:self-auto"
