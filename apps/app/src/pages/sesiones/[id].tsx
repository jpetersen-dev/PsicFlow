...
export default function SesionFicha() {
  ...
  const handleDeleteSession = async () => {
    ...
    try {
      const tenantId = localStorage.getItem('active-tenant-id');
      // If there is an associated Google Event, try to delete it from Google Calendar via API Route
      if (session.google_event_id && tenantId) {
        try {
      ...
      // Delete the session from Supabase (cascades note/audios/files)
      ...
      router.push(`/pacientes/${session.patient_id}`);
    } catch (err: any) {
        ...
        <div className="bg-bg-card border border-border-color rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className="border border-danger/35 hover:bg-danger/10 text-danger px-4 py-2.5 rounded-lg text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5"
            >
          ...
          <div className="bg-bg-card border border-border-color w-full max-w-md rounded-xl shadow-2xl flex flex-col">
            ...
            <div className="p-6 space-y-3">
              <p className="text-xs text-text-secondary leading-normal">
                ¿Estás seguro de que deseas eliminar permanentemente esta sesión clínica del día <strong>{session?.date_session}</strong> a las <strong>{session?.time_session?.slice(0, 5)}</strong> hrs?
              </p>
              <p className="text-xs text-danger font-medium leading-normal bg-danger/5 border border-danger/25 p-3 rounded-lg flex items-start gap-2">
                ...
                <span>Esto eliminará la evolución clínica SOAP asociada, los archivos de audio vinculados y las grabaciones. Si está sincronizada con Google Calendar, el evento también será removido de tu agenda.</span>
              </p>
            </div>
