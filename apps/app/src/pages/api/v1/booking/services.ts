...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { data: services, error: fetchErr } = await supabase
      .from('services')
      .select('id, title, id_slug, duration_minutes, price, currency, image_url, desc, clinical_approach, seo_description, icon, color, alternate_prices, seo_title, json_ld, what_we_work')
