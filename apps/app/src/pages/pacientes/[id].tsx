...
export default function PacienteFicha() {
  ...
  const handleDeletePatient = async () => {
    if (!id || deleteConfirmationText !== patient?.full_name) return;
    setSubmitting(true);
    try {
      ...
      alert('Error al eliminar el paciente: ' + (err.message || 'Error desconocido'));
    } finally {
      setSubmitting(false);
  ...
  const handleDeleteSession = async () => {
    ...
    setSubmitting(true);
    try {
      const sess = sessions.find(s => s.id === deleteSessionId);
      const tenantId = localStorage.getItem('active-tenant-id');
      if (sess && sess.google_event_id && tenantId) {
        try {
          ...
          console.error('UI: Failed to trigger Google Calendar event deletion:', gErr.message);
        }
      }
      ...
      alert('Error al eliminar la sesión: ' + (err.message || 'Error desconocido'));
    } finally {
      setSubmitting(false);
  ...
  return (
    <>
            ...
            {!isEditing ? (
              <>
                ...
                <button 
                  ...
                  className="px-4 py-2 border border-danger/35 hover:bg-danger/10 text-danger rounded-lg text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5"
                >
                    ...
                    <div key={sess.id} className="pt-4 first:pt-0 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-xs">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button 
                            onClick={() => {
                              setDeleteSessionId(sess.id);
                              setIsDeleteSessionOpen(true);
                            }}
                            className="p-2 border border-danger/25 bg-danger/5 hover:bg-danger/10 text-danger rounded-lg transition-all cursor-pointer"
      ...
      {isDeletePatientOpen && (
        <div className="fixed inset-0 bg-bg-primary/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-bg-card border border-border-color w-full max-w-md rounded-xl shadow-2xl flex flex-col">
            <div className="flex items-center gap-3 px-6 py-4 border-b border-border-color bg-danger/5">
              ...
              <div>
                ...
                <p className="text-[11px] text-text-secondary">Esta acción es irreversible y borrará todo su historial clínico.</p>
              </div>
            </div>
            <div className="p-6 space-y-4">
              <p className="text-xs text-text-secondary leading-normal">
                Se eliminarán permanentemente el perfil de <strong>{patient?.full_name}</strong>, su ficha clínica, todas sus {sessions.length} sesiones agendadas, evoluciones clínicas, diagnósticos y archivos asociados.
              </p>
              <div>
                <label className="block text-[11px] text-text-secondary mb-1.5 font-semibold">
                  Para confirmar, escribe el nombre completo del paciente (<strong>{patient?.full_name}</strong>):
                </label>
                <input 
                  type="text" 
                  ...
                  className="w-full bg-bg-input border border-border-color rounded-lg px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-danger/40 focus:ring-1 focus:ring-danger/20"
                />
              </div>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border-color bg-bg-input/20">
              <button 
                onClick={() => {
                  setIsDeletePatientOpen(false);
                  setDeleteConfirmationText('');
                }}
                className="px-4 py-2 border border-border-color hover:bg-bg-card-hover text-xs font-semibold rounded-lg text-text-secondary transition-all cursor-pointer"
              >
                Cancelar
              </button>
              <button 
                disabled={deleteConfirmationText !== patient?.full_name || submitting}
                onClick={handleDeletePatient}
                className="px-4 py-2 bg-danger hover:bg-danger/90 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-lg text-xs font-bold transition-all cursor-pointer"
              >
                {submitting ? 'Eliminando...' : 'Sí, Eliminar Paciente'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-bg-card border border-border-color w-full max-w-md rounded-xl shadow-2xl flex flex-col">
            <div className="flex items-center gap-3 px-6 py-4 border-b border-border-color bg-danger/5">
              ...
              <div>
                ...
                <p className="text-[11px] text-text-secondary">Esta acción es irreversible.</p>
              </div>
            </div>
            <div className="p-6 space-y-3">
              <p className="text-xs text-text-secondary leading-normal">
                ¿Estás seguro de que deseas eliminar permanentemente esta sesión clínica del día <strong>{sessions.find(s => s.id === deleteSessionId)?.date_session}</strong>?
              </p>
              <p className="text-xs text-danger font-medium leading-normal bg-danger/5 border border-danger/25 p-3 rounded-lg flex items-start gap-2">
                <AlertTriangle className="w-4 h-4 text-danger shrink-0 mt-0.5" />
                <span>Esto eliminará la evolución SOAP asociada, audios y archivos vinculados. Si está sincronizada con Google Calendar, el evento será removido de tu agenda.</span>
              </p>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border-color bg-bg-input/20">
              <button 
              ...
              <button 
                ...
                className="px-4 py-2 bg-danger hover:bg-danger/90 text-white rounded-lg text-xs font-bold transition-all cursor-pointer"
              >
                {submitting ? 'Eliminando...' : 'Sí, Eliminar Sesión'}
              </button>
            </div>
          </div>
        </div>
