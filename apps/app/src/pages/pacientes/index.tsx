...
export default function Pacientes() {
  ...
  const [crmTab, setCrmTab] = useState<'pacientes' | 'prospectos'>('pacientes');
  ...
  const fetchPatients = async () => {
    ...
    try {
      ...
      if (!error && data) {
        ...
        // Compute prospects count
        const prospects = data.filter(p => p.status === 'prospecto').length;
  ...
  useEffect(() => {
    ...
    // Filter by CRM Tab (clinical vs prospects)
    if (crmTab === 'pacientes') {
      result = result.filter(p => p.status !== 'prospecto');
    } else {
      result = result.filter(p => p.status === 'prospecto');
    }
    ...
    if (sortOrder === 'name_asc') {
      ...
      result.sort((a, b) => {
        const fa = a.ficha_id_num || '';
        const fb = b.ficha_id_num || '';
  ...
  }, [search, statusFilter, sortOrder, patients, crmTab]);
  ...
  return (
    <>
      ...
      <div className="space-y-stack-lg">
        ...
        {/* CRM Tabs */}
        <div className="flex border-b border-outline-variant/30 gap-6 mt-6">
          <button
            onClick={() => {
              setCrmTab('pacientes');
              setStatusFilter('all');
        ...
        <div className="bg-surface-container-lowest rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.03)] border border-outline-variant/30 overflow-hidden">
          ...
          <div className="px-gutter py-4 border-b border-outline-variant/20 flex flex-col md:flex-row items-center justify-between gap-4 bg-surface-container-low/20">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
              {/* Filter (Only show for clinical patients) */}
              {crmTab === 'pacientes' && (
